...
async function generateOverlay() {
  try {
    ...
    for (const checkbox of thresholdCheckboxes) {
      const giftName = checkbox.value;
      // Load threshold config to get display info
      const thresholdResult = await window.sniAPI.loadThresholdConfigs();
      if (thresholdResult.success && thresholdResult.thresholds[giftName]) {
        const config = thresholdResult.thresholds[giftName];
        const displayName = giftName === '__VALUE_TOTAL__' ? 'Total Coin Value' : giftName;
        selectedThresholds.push({
          giftName,
          displayName,
          target: config.target,
          action: config.description || config.action
        });
      }
    }

    // Generate HTML content
    const html = generateOverlayHTML(gifts, width, height, stagger, pause, continuousLoop, spacing, selectedThresholds);
